...
bool Monitor_light = true;   // set to true or false to enable or disable BH1750 values through serial
...
///// BH1750 Setup /////
void setupBH1750() {
  // BH1750 default mode is continuous high-res
  if (lightMeter.begin(BH1750::CONTINUOUS_HIGH_RES_MODE)) {
    logMessage("BH1750 initialized", Monitor_log);
  } else {
    logMessage("Error initializing BH1750", Monitor_log);
  }
}
...
void readBH1750() {
  luxReading = lightMeter.readLightLevel();

  if (luxReading < 0) {
    logMessage("Error reading BH1750 light level!", Monitor_light);
  } else if (Monitor_light) {
...
void writeThingSpeak() {
  ...
  ThingSpeak.setField(7, luxReading);
